...
export const startSplendourGame = (obj) => {
  ...
  <form class="form">
    <div class="wellcome__game">Начинаем новую игру в Роскошь!</div>
    <div>
  ...
  start.addEventListener('click', (even) => {
    ...
    if (playersList.value.split(',').length < 2 || playersList.value.split(',').length > 4) {
      ...
      playingField.innerHTML = `<div>Роскошь</div><br>
        <div>Не знаешь, как пользоваться таблицей?</div>
      `;

      infoModalWindow(`<div>Жми INFO</div>`, `<div>После определения победителя поставьте 1 в активную
      ячейку таблицы под именем победившего игрока и нажмите кнопку "ок" справа от вписанного результата.
      По окончанию игры используйте кнопку "Закончить игру".</div>
      <div class="loss__btn">
      <div id="btn">Понятно!</div>
      </div>`, playingField);
